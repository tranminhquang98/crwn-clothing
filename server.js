...
app.use(express.json()); //parse input
app.use(express.urlencoded({ extended: true })); //urlencoded is a way for us to make sure that the URL strings we're getting in and we're passing out do not contain things lkke spaces or symbols

app.use(cors()); //Cross-Origin Request so we're actually able to properly make requests to our backend server

if (process.env.NODE_ENV === 'production') {
  app.use(compression());
  app.use(enforce.HTTPS({ trustProtoHeader: true })); //For
